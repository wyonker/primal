...
function Display_Servers() {
	...
	echo '<button class="open-button" onclick="openForm()">Select Gateway</button>';
	echo '<div class="form-popup" id="myForm">';
	echo '<form action="index.php" class="form-container">';
    echo '<h1>Select Servers to View</h1>';
	echo '<select name="Servers" multiple required>';
...
function Sanatize_Input($strInput, $intType, $intLength) {
	global $strReturn;

	$intError=0;
	if(!is_numeric($intType))
		$intError=1;
	if(!is_numeric($intLength))
		$intError=2;
	if(($intLength < 3) || ($intLength > 255))
		$intError=2;
	if(strlen($strInput) > $intLength) {
		$intError=3;
		$strInput=substr($strInput, 0, ($intLength -1));
	}
	$strTemp=preg_replace("/[^a-zA-Z0-9 \-_]/", "", $strInput);
	if(strcmp($strInput, $strTemp) !== 0)
		$intError=4;

	$strInput=$strTemp;
	$strInput=strtoupper($srInput);
