...
QUEUELENGTH=`echo "select count(*) from ticket where destination = '${PRIDESTAEC[$NUMDEST]}' order by ticket_num;"|mysql -N -u root primal`
...
then
	...
	while [ $DONE -ne 1 ]
	do
		HASTICKET=`echo "select count(*) from ticket where destination = '${PRIDESTAEC[$NUMDEST]}' and proc_id = '$THISPID';"|mysql -N -u root primal`
		PROCID=`echo "select proc_id from ticket where destination = '${PRIDESTAEC[$NUMDEST]}' order by ticket_num limit 1;"|mysql -N -u root primal`
		...
		then
			...
			then
				...
				echo "delete from ticket where destination = '${PRIDESTAEC[$NUMDEST]}' and proc_id = '$PROCID';"|mysql -N -u root primal
			...
			echo "insert into ticket set destination = '${PRIDESTAEC[$NUMDEST]}', proc_id = '$THISPID';"|mysql -N -u root primal
		...
		then
			...
			echo "delete from ticket where destination = '${PRIDESTAEC[$NUMDEST]}' and proc_id = '$THISPID';"|mysql -N -u root primal
