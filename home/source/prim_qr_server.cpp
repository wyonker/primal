...
std::string fCreateJson(std::string strPrimalID, std::size_t intPrefetchPendingID, std::size_t intLC) {
    ...
    if(*mysql_error(mconnect2)) {
        strLogMessage=mysql_error(mconnect2);
        strLogMessage+="strQuery9 = " + strQuery;
        fWriteLog(strLogMessage, conf1.primConf[strRecNum + "_PRILOGDIR"] + "/" + conf1.primConf[strRecNum + "_PRILFQR"]);
        exit(-1);
    }
    ...
    if(*mysql_error(mconnect2)) {
        strLogMessage=mysql_error(mconnect2);
        strLogMessage+="strQuery10 = " + strQuery;
        fWriteLog(strLogMessage, conf1.primConf[strRecNum + "_PRILOGDIR"] + "/" + conf1.primConf[strRecNum + "_PRILFQR"]);
        exit(-1);
    }
    strMRN="0";
...
std::size_t fSelectPriors(std::string strPrimalID, std::size_t intPrefetchPendingID) {
    ...
    mysql_query(mconnect2, strQuery.c_str());
    if(*mysql_error(mconnect2)) {
        strLogMessage = strPrimalID + " QR " + " QR query13 = " + strQuery;
        fWriteLog(strLogMessage, conf1.primConf[strRecNum + "_PRILOGDIR"] + "/" + conf1.primConf[strRecNum + "_PRILFQR"]);
        //std::cout << strLogMessage << std::endl;
    ...
    */
    if(intDone == 0) {
        ...
        while ((row = mysql_fetch_row(result))) {
            ...
            mysql_query(mconnect2, strQuery.c_str());
            if(*mysql_error(mconnect2)) {
                strLogMessage = strPrimalID + " QR " + " QR query14 = " + strQuery;
                fWriteLog(strLogMessage, conf1.primConf[strRecNum + "_PRILOGDIR"] + "/" + conf1.primConf[strRecNum + "_PRILFQR"]);
                //std::cout << strLogMessage << std::endl;
                strLogMessage = strPrimalID + " QR " + " SQL Error: " + mysql_error(mconnect2);
                fWriteLog(strLogMessage, conf1.primConf[strRecNum + "_PRILOGDIR"] + "/" + conf1.primConf[strRecNum + "_PRILFQR"]);
            ...
            if(!fs::exists(strFullPath)) {
                ...
                if(intPrefetchPendingID != 0) {
                    ...
                    if(intUseRis != 1) {
                        ...
                        fWriteLog(strLogMessage, conf1.primConf[strRecNum + "_PRILOGDIR"] + "/" + conf1.primConf[strRecNum + "_PRILFQR"]);
                        //mtx.lock();
                        strQuery = "select sCaseID from study where puid = '" + strPrimalID + "' limit 1;";
                        ...
                        if(*mysql_error(mconnect)) {
                            ...
                            if(result) {
                                ...
                                if(intNumRows > 0) {
                                    ...
                                    strLogMessage = strPrimalID + " QR  Got CaseID " + strCaseID;
                                    fWriteLog(strLogMessage, conf1.primConf[strRecNum + "_PRILOGDIR"] + "/" + conf1.primConf[strRecNum + "_PRILFQR"]);
                                    if(strCaseID.compare("0") != 0) {
                                        strJson = fCreateJson(strPrimalID, intPrefetchPendingID, intLC);
                                        strLogMessage=strPrimalID + " QR  strJson = " + strJson;
                                        fWriteLog(strLogMessage, conf1.primConf[strRecNum + "_PRILOGDIR"] + "/" + conf1.primConf[strRecNum + "_PRILFQR"]);
                        ...
                        //mtx.unlock();
                        strLogMessage=strPrimalID + " QR  Updating qrstatus to Hold.";
                        fWriteLog(strLogMessage, conf1.primConf[strRecNum + "_PRILOGDIR"] + "/" + conf1.primConf[strRecNum + "_PRILFQR"]);
            ...
            if(intNumRows > 0) {
                while((row = mysql_fetch_row(result))) {
                    ...
                    if(intUseRis != 1) {
                        ...
                        fWriteLog(strLogMessage, conf1.primConf[strRecNum + "_PRILOGDIR"] + "/" + conf1.primConf[strRecNum + "_PRILFQR"]);
                        strQuery = "select sCaseID from study where puid = '" + strPrimalID + "';";
                        strLogMessage = strPrimalID + " QR  strQuery = " + strQuery;
                        fWriteLog(strLogMessage, conf1.primConf[strRecNum + "_PRILOGDIR"] + "/" + conf1.primConf[strRecNum + "_PRILFQR"]);
                        mysql_query(mconnect, strQuery.c_str());
                        if(*mysql_error(mconnect)) {
                            strLogMessage=mysql_error(mconnect);
...
void fCheckStartup() {
    ...
    std::string strPID, strQuery, strLogMessage, strRecNum, strCmd, strPrimalID;
        ...
        if(*mysql_error(mconnect2)) {
            ...
            if(result) {
                ...
                if(intNumRows > 0) {
                    while((row = mysql_fetch_row(result))) {
                        strPrimalID = row[0];
                        strCmd = "/home/dicom/processing/" + strPrimalID + " 3";
                        strLogMessage = strPrimalID + " QR  Requeing message:  " + strCmd;
                        fWriteLog(strLogMessage, conf1.primConf[strRecNum + "_PRILOGDIR"] + "/" + conf1.primConf[strRecNum + "_PRILFQR"]);
...
int main() {
    ...
    //if(iprimConf == conf1.primConf.end()) {
    //    conf1.primConf[strRecNum + "_PRILFQR"]="qr.log";
    //}
    //std::thread th(fPollRIS);
    fCheckStartup();
    while(1) {
        strMessage=fGetMessage("/prim_qr");
        fQRmain(strMessage);
    ...
    while(1) {
        ...
        if(vecThreads.size() < intMaxThreads) {
            vecThreads.emplace_back(fQRmain, strMessage);
            intNumThreads++;
            ...
            while(intFoundThread == 0) {
                for (long unsigned int i=0; i<vecThreads.size(); i++) {
                    if(vecThreads[i].joinable()) {
                        vecThreads[i].join();
                        std::thread th(fQRmain, strMessage);
