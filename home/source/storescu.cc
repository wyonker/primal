...
#include <iostream> //WMY
#include <glob.h> //WMY
#include <map> //WMY
#include <string> //WMY
#include <dirent.h> //WMY
...
class ConfFile
	{
		...
		int StartSend(std::string strPath, std::string strPRILOGDIR, std::string strPRILFOUT, PatientData& pData);
		int EndSend(std::string strRec, std::string strPath);
	...
	int ConfFile::ValidateConf(void)
	{
		...
		{
			cout << "Error:  PRIPORT is : " << intValue << " and must be a number and must be between 100 and 100,000." << endl;
			return 1;
		}
		//PRIDESTPORT0
...
int ConfFile::PriError(std::string strErrCode, const char* FileName) {
	...
	std::string strTemp2, strQuery, strIlocation;
	...
	//Create full path to put into the database
	strIlocation = this->primConf["PRISENT"] + "/" + pData.strDirName;

	strQuery = "update send set tendsend='" + GetDate() + "', serror='1', complete='1' where send.puid='" + pData.strDirName + "' and tdest='" + pData.strDest + "';";
...
int ConfFile::PriLog(std::string strLog) {
	...
	LogFile << strLogDate << "   PRIMAL: " << pData.strDirName << " Destination: " << pData.strDest << "  "<< strLog << " for Patient: " << pData.strPName << "  MRN: " << pData.strMRN << " Accession#: " << pData.strACCN << " with " << pData.intNumFiles << " images." << endl;
	LogFile.close();
	return 0;
}

int ConfFile::StartSend(std::string strPath, std::string strPRILOGDIR, std::string strPRILFOUT, PatientData& pData) {
	size_t intPOS;
	std::string strDirName, strTemp, strTimeStamps, strQuery, strPath3=strPath;
	glob_t gl;
	time_t tmStartSendSec = time(0);
	std::ifstream TSFile;
	std::vector<std::string> vecFiles, vecTimeStamps, vecSplit;
	FILE *in;
	char * pos;
	char buff[512], chrHostname[1024];
	
	pData.intIsError=0;
	intPOS = strPath.find_last_of("/");
	if ( string::npos != intPOS ) {
		pData.strPath = strPath.substr(0, intPOS);
		strTemp = pData.strPath.substr(0, intPOS);
		intPOS = strTemp.find_last_of("/");
		pData.strDirName = strTemp.substr(intPOS+1);
	} else {
		cout << "Could not get correct path.  Exiting..." << endl;
		return 1;
	}

	gethostname(chrHostname, 1024);
	std::string strPath2=pData.strPath+"/*";
	pData.strStartDate=GetDate();
	this->PriLog("Started Sending");
	strTemp = "dcmdump " + strPath3 + " 2>&1";
	in = popen(strTemp.c_str(), "r");
	if(in == NULL) {
		return 1;
	} else {
		while(fgets(buff, sizeof(buff), in)!=NULL) {
			pos = strstr(buff, "(0010,0010)");
			if(pos != NULL) {
				pData.strPName = buff;
				intPOS = pData.strPName.find("[");
				pData.strPName=pData.strPName.substr(intPOS + 1);
				intPOS = pData.strPName.find("]");
				pData.strPName=pData.strPName.substr(0,intPOS);
			}
			pos = strstr(buff, "(0010,0020)");
			if(pos != NULL) {
				pData.strMRN = buff;
				intPOS = pData.strMRN.find("[");
				pData.strMRN=pData.strMRN.substr(intPOS + 1);
				intPOS = pData.strMRN.find("]");
				pData.strMRN=pData.strMRN.substr(0,intPOS);
			}
			pos = strstr(buff, "(0008,0050)");
			if(pos != NULL) {
				pData.strACCN = buff;
				intPOS = pData.strACCN.find("[");
				pData.strACCN=pData.strACCN.substr(intPOS + 1);
				intPOS = pData.strACCN.find("]");
				pData.strACCN=pData.strACCN.substr(0,intPOS);
			}
		}
		pclose(in);
	}
	this->PriLog("Started sending study ");
	strQuery = "insert into send (puid, sservername, tdest, tstartsend, timages) values ('" + pData.strDirName + "', '" + chrHostname + "', '" + pData.strDest + "', '" + GetDate() + "', '" + std::to_string(pData.intNumFiles) + "');";
	mysql_query(mconnect, strQuery.c_str());

	return 0;
}

int ConfFile::EndSend(std::string strRec, std::string strPath) {
	std::vector<string> vecTimeStamps;
	std::string strTemp, strTemp2, strQuery, strIlocation;
	std::map<std::string,std::string>::iterator itConf;
	std::ofstream TSfile2;
	std::ifstream TSFile;
	
	if(pData.intIsError > 0)
	{
		return 1;
	}
	pData.strEndDate=GetDate();
	...
	strIlocation = this->primConf["PRISENT"] + "/" + pData.strDirName;
	strQuery = "update send set tendsend='" + GetDate() + "', serror='0', timages='" + std::to_string(pData.intNumFiles) + "', complete = '1' where send.puid='" + pData.strDirName + "' and tdest='" + pData.strDest + "';";
	mysql_query(mconnect, strQuery.c_str());
    //All done.
...
{
	std::size_t intLC1=1, intFound=0, intARGC;
	...
	std::string strDirPath = argv[argc -1];
	//Need to convert the pointer to a usable variable
	intARGC = argc-1;
	//Loop through passed values until I find the right one (or not)
	while(intLC1 < intARGC) {
		//I don't seem to be able to do a compare on argv[intLC1] so I'll put it into a variable
		if(strcmp(argv[intLC1], "-aec") == 0) {
			//Found -aec, the next value should be the actual title.  Let's set it.
			intLC1++;
			pData.strDest += argv[intLC1];
			//Found the right one so don't need to make it UNKNOWN
			intFound=1;
		} else {
			intLC1++;
		}
	...
	if(intFound == 0) {
		//We were unable to find the AEC so let's set it to something
		pData.strDest = "UNKNOWN";
	}
	std::string strSCP = GetSCP(strDirPath);
	pData.strRec = strSCP;
	if (strSCP == "*") {
		cout << "Error:  Could not determine which receiver to use.  Exiting..." << endl;
		return 1;
	}
	...
	if (intReturn == 1) {
		cout << "Error:  Could not initialize ConfFile class.  Exiting..." << endl;
		return 1;
	}

	intReturn=conf1.StartSend(strDirPath, conf1.primConf["PRILOGDIR"], conf1.primConf["PRILFOUT"], pData);
	if (intReturn == 1) {
		cout << "Error:  Could not start logging.  Exiting..." << endl;
		return 1;
	}
  ...
  cmd.addGroup("transport layer security (TLS) options:");
    ...
    while (if_iter != if_last)
    {
      ...
      if (OFStandard::fileExists(currentFilename))
      {
        if (opt_proposeOnlyRequiredPresentationContexts)
        {
          if (!findSOPClassAndInstanceInFile(currentFilename, sopClassUID, sopInstanceUID))
          {
            ...
            conf1.PriError("missing SOP class (or instance) in file: ", currentFilename); //WMY
          ...
          else if (!dcmIsaStorageSOPClassUID(sopClassUID))
          {
            ...
            conf1.PriError("unknown storage SOP class in file: ", currentFilename); //WMY
            if (opt_haltOnUnsuccessfulStore)
            {
              OFLOG_FATAL(storescuLogger, errormsg);
              exit(1);
      ...
      else
      {
        ...
        errormsg += currentFilename;
        conf1.PriError("cannot access file: ", currentFilename); //WMY
        if (opt_haltOnUnsuccessfulStore)
        {
          OFLOG_FATAL(storescuLogger, errormsg);
          exit(1);
    ...
    if (cond.bad()) {
      if (cond == DUL_ASSOCIATIONREJECTED) {
        ...
        conf1.PriError("Association Rejected:", currentFilename); //WMY
        return 1;
      } else {
        OFLOG_FATAL(storescuLogger, "Association Request Failed: " << DimseCondition::dump(temp_str, cond));
        conf1.PriError("Association Request Failed: ", currentFilename); //WMY
        return 1;
      }
    }
    ...
    if (ASC_countAcceptedPresentationContexts(params) == 0) {
      ...
      conf1.PriError("No Acceptable Presentation Contexts", currentFilename); //WMY
      return 1;
    }
    ...
    while ((iter != enditer) && cond.good())
    {
      ...
      //OFLOG_INFO(storescuLogger, iter); //WMY
      ++iter;
    }
    ...
    if (cond == EC_Normal)
    {
      if (opt_abortAssociation) {
        ...
        if (cond.bad()) {
          ...
          conf1.PriError("Association Abort Failed: ", currentFilename); //WMY
          return 1;
        }
      } else {
        /* release association */
        ...
        if (cond.bad())
        {
          ...
          conf1.PriError("Association Release Failed: ", currentFilename); //WMY
          return 1;
        }
      }
    }
    else if (cond == DUL_PEERREQUESTEDRELEASE)
    {
      ...
      OFLOG_INFO(storescuLogger, "Aborting Association");
      cond = ASC_abortAssociation(assoc);
      conf1.PriError("Protocol Error: Peer requested release (Aborting)", currentFilename); //WMY
      if (cond.bad()) {
        OFLOG_ERROR(storescuLogger, "Association Abort Failed: " << DimseCondition::dump(temp_str, cond));
        return 1;
      }
    ...
    else
    {
      ...
      OFLOG_INFO(storescuLogger, "Aborting Association");
      cond = ASC_abortAssociation(assoc);
      conf1.PriError("Store SCU Failed: ", currentFilename); //WMY
      if (cond.bad()) {
        OFLOG_ERROR(storescuLogger, "Association Abort Failed: " << DimseCondition::dump(temp_str, cond));
        return 1;
      }
    ...
    if(exitCode != 0) {
    	conf1.PriError("Unknown Error: Please check log files", currentFilename); //WMY
    } else {
        strMessage = "Successfully sent: ";
        conf1.PriLog(strMessage); //WMY
    }
    conf1.EndSend(strSCP, strDirPath); //WMY
    return exitCode;
}
   ...
   */
{
  ...
  else
  {
    ...
    //conf1.PriError("Store Failed, file: ", currentFilename); //WMY
    }
